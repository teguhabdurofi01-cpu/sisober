...
export default async function Dashboard({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div className="command-dashboard">
    <header className="command-toolbar"><div><h1>Safety intelligence</h1><p>{isContractor ? `Outstanding untuk ${companies?.map((item: any) => item.code).join(", ") || "kontraktor Anda"}` : "Ringkasan keselamatan unit dan perilaku pengemudi"}</p></div><div className="command-actions"><time className="command-date" dateTime={today.toISOString()}><span>Hari ini</span><strong>{weekday}</strong><small>{fullDate}</small></time>{isAdmin && <a className="command-btn secondary" href="/api/export/findings"><Download size={15}/>Export</a>}<Link className="command-btn" href={isContractor ? "/findings?view=outstanding" : "/findings/new"}>{isContractor ? "Lihat tugas" : <><Plus size={15}/>Temuan baru</>}</Link></div></header>
